...
```
ls /boot/vmlinuz-* | grep '6.1.0-10..-oem' | awk -F"-" '{split($0, a, "-"); version=a[3]; if (version>max) {max=version; kernel=a[2] "-" a[3] "-" a[4]}} END{print kernel}'
```

Right now, this is **6.1.0-1024-oem** - but this may evolve in the future.
...
``
GRUB_DEFAULT="Advanced options for Ubuntu>Ubuntu, with Linux 6.1.0-1024-oem"
``
